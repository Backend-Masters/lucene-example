...
/**
 * Simple Lucene Index & Search Example for Korean Analyzer
 *
 * 대한민국 헌법 본문으로 검색
 * reference : http://www.lucenetutorial.com/lucene-in-5-minutes.html
 */
public class SimpleKoreanDemo {

	public static void main(String[] args) throws Exception {

		InputStream is = SimpleKoreanDemo.class.getClassLoader().getResourceAsStream("대한민국_헌법.txt");
		...
		search("국민", analyzer, DirectoryReader.open(directory));
		search("주권", analyzer, DirectoryReader.open(directory));
		search("자유", analyzer, DirectoryReader.open(directory));
		search("대통령", analyzer, DirectoryReader.open(directory));
	}
